...
dep = moab.add_job("merge" , cmds.final_merge, "", re_final, final_deps)
...
if config.global_input_MP == 0 and config.global_bwaaln == 0
  s_deps << moab.add_job("stats", cmds.stats_frag, "", re_stats, [dep])
else
  s_deps << moab.add_job("stats_F3", cmds.stats_f3, "", re_stats, [dep])
  s_deps << moab.add_job("stats_R3", cmds.stats_r3, "", re_stats, [dep])
end

# Run BAM Reads Validation
dep = moab.add_job("bam_reads_val", cmds.bam_reads_validator, "", re_stats, [dep])

# Run Capture Stats
caps_dir = config.capture_stats_dir
if config.global_input_CAP == 1
  Dir.mkdir(caps_dir)
  dep = moab.add_job("capture_stats", cmds.capture_stats, "", re_cap, s_deps)
end
